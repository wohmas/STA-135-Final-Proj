...
In the box plots, outliers in the manufacturing and population variables immediately stand out, as they appear to be very far from their distributions. This causes their box plots to be drawn very narrowly to fit the full distribution. Extreme values are also apparent across multiple scatterplots. Additionally, a negative correlation between temperature and SO2 can be seen, as well as a positive correlation between the population and manufacturing variables and another positive correlation between the precipitation and days of precipitation variables. This will come up again later when we perform the principal component analysis.


With respect to outliers, the process of identification in the multivariate setting is surprisingly challenging due to the nature of higher dimensional data, and more advanced methods of outlier detection appear to be outside of the scope of this course. However, a somewhat rudimentary method obtained by [citations] involves the use of a bivariate box plot, which is applied to the manufacturing and population variables to further investigate their outliers:
...
In order to answer the questions about which variables are most important for predicting air pollution levels and how the cities rank against each other, we use the multivariate analysis method of PCA. PCA is a way of re-expressing the data in terms of new vectors, where each vector is orthogonal to the the other vectors and therefore can be considered independent. These vectors are generated from the variance of the data set, so that their orthogonal dimensions also capture its variance. 
...
We also make another decision for the sake of interpretability: Noting the negative association between the temperature variable and the SO2 variable that can be seen in the scatterplot matrix, we flip the sign of the temperature variable, so that "increasing" values in the negative temperature track with increasing values in the other variables, and thus makes correlations between the principle components easier to see.
